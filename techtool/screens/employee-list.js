...
export function renderEmployeeList(container, state, navigate) {
  ...
  container.innerHTML = `
    <div class="screen">
      ...
      <div class="search-wrap"><input id="emp-search" type="search" class="search-input" placeholder="Search by name…" /></div>
      ${allResolved ? `<div class="emp-complete-banner"><p>✓ All workers resolved. Ready to submit visit.</p><button class="btn btn-primary btn-sm" id="btn-submit-packet">Submit Packet to Office →</button></div>` : ''}
      <div id="emp-list" class="emp-list">${buildList(employees)}</div>
      ...
      <div id="add-emp-form" class="add-emp-form" style="display:none">
        <div class="form-row"><div class="form-group"><label>First Name *</label><input id="new-first" type="text" /></div><div class="form-group"><label>Last Name *</label><input id="new-last" type="text" /></div></div>
        <div class="form-row"><div class="form-group"><label>Birthdate *</label><input id="new-dob" type="date" /></div><div class="form-group"><label>Job Title</label><input id="new-title" type="text" /></div></div>
        <div style="display:flex;gap:8px;margin-top:12px"><button class="btn btn-primary btn-sm" id="btn-add-emp-submit">Add to Packet</button><button class="btn btn-secondary btn-sm" id="btn-add-emp-cancel">Cancel</button></div>
      </div>
    </div>`;
...
function buildList(employees) {
  return employees.map(emp => {
    ...
    return `<div class="emp-row ${done ? 'emp-row--done' : ''} ${skipped ? 'emp-row--skipped' : ''}" data-emp-id="${emp.employee_id}"><div class="emp-row__info"><div class="emp-row__name">${esc(emp.last_name)}, ${esc(emp.first_name)}</div><div class="emp-row__meta">${esc(emp.job_title || '')} ${skipped ? '· SKIPPED' : ''}</div></div><div class="emp-row__right">${done ? '<span class="badge badge-success">✓ Done (Edit)</span>' : skipped ? '<span class="badge">Skipped</span>' : '<span class="badge badge-neutral">Pending</span>'}${!done && !skipped ? `<button class="emp-row__skip" data-skip-id="${emp.employee_id}">Skip</button>` : ''}</div></div>`;
  }).join('');
}
...
function attachRowHandlers(container, employees, packet, state, navigate) {
  container.querySelectorAll('.emp-row').forEach(row => {
    row.onclick = (e) => {
      ...
      // UNIFIED EDIT LOGIC: No more key mapping needed!
