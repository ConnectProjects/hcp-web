// Version 2.2 - Robust Data Tools
import { query, transaction } from '../db/sqlite.js'

export function renderDataTools(container, state, navigate) {
  const loadData = () => {
    const companies = query("SELECT company_id, name FROM companies ORDER BY name ASC");

    container.innerHTML = `
      <div class="page">
        <div class="page-header">
          <h1>Data Management Tools</h1>
          <button class="btn btn-outline" id="btn-refresh-tools">🔄 Refresh Data</button>
        </div>
            ...
            <div class="form-card">
              ...
              <div class="form-group">
                ...
                <select id="emp-src-co" class="search-input">
                  ...
                  ${companies.map(c => `<option value="${c.company_id}">${c.name} [ID: ${c.company_id}]</option>`).join('')}
                </select>
              </div>
              <div class="form-group">
  ...
  const setupEventListeners = () => {
    // Refresh Button
    container.querySelector('#btn-refresh-tools').onclick = () => renderDataTools(container, state, navigate);

    // Tabs
    container.querySelectorAll('.tab-btn').forEach(btn => {
      btn.onclick = () => {
        container.querySelectorAll('.tab-btn').forEach(b => { b.style.borderBottom = "none"; b.style.color = "#666"; });
    ...
    const updateLocs = (coId, selectId) => {
      ...
      // REMOVED "active = 1" to ensure we find everything
    ...
    container.querySelector('#select-all-emp').onchange = (e) => {
    ...
    btnMoveEmp.onclick = () => {
      ...
      if (confirm(`Move ${selected.length} employees?`)) {
          ...
          renderDataTools(container, state, navigate);
      }
    };
    ...
    container.querySelector('#btn-execute-loc-move').onclick = () => {
